...
const MIN_DEPLOY_BALANCE_BNB = "0.05";

async function main() {
  const PLATFORM_WALLET = process.env.BSC_PLATFORM_WALLET || DEFAULT_PLATFORM_WALLET;
  const ORACLE_ADDRESS = process.env.BSC_ORACLE_ADDRESS || DEFAULT_ORACLE_ADDRESS;
  ...
  const minBalance = ethers.parseEther(MIN_DEPLOY_BALANCE_BNB);
  if (balance < minBalance) {
    throw new Error(
      `Deployer balance ${ethers.formatEther(balance)} BNB is below required ${MIN_DEPLOY_BALANCE_BNB} BNB. ` +
      `Fund ${deployer.address} on BSC mainnet and re-run.`
    );
  }

  const Factory = await ethers.getContractFactory("Skills2CryptoEscrow");

  console.log("Deploying Skills2CryptoEscrow V2 to BSC mainnet...");
  ...
  console.log("\nSet BSC_ESCROW_ADDRESS=" + address);
  console.log("=================================\n");
}
